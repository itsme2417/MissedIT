...
DrawPlayer(void *thisptr, void *context, void *state, const ModelRenderInfo_t &pInfo, matrix3x4_t *pCustomBoneToWorld) {
    ...
    switch (chamsType) {
        ...
        case ChamsType::ANIMATED:
            visible_material = material->FindMaterial("models/inventory_items/music_kit/darude_01/mp3_detail",
                                                      TEXTURE_GROUP_MODEL);
            hidden_material = materialChamsFlatIgnorez;
        ...
        case ChamsType::PULSE:
            visible_material = material->FindMaterial("models/inventory_items/dogtags/dogtags_outline",
                                                      TEXTURE_GROUP_MODEL);
            hidden_material = materialChamsFlatIgnorez;
            wap = false;
            break;
...
//		var.SetVecValue(Settings::ESP::Chams::localplayerColor.Color(entity).Value.x, Settings::ESP::Chams::localplayerColor.Color(entity).Value.y, Settings::ESP::Chams::localplayerColor.Color(entity).Value.z);
    ...
    visible_material->SetMaterialVarFlag(MATERIAL_VAR_WIREFRAME,
                                         chamsType == ChamsType::WIREFRAME || chamsType == ChamsType::ANIMATED);
...
DrawFake(void *thisptr, void *context, void *state, const ModelRenderInfo_t &pInfo, matrix3x4_t *pCustomBoneToWorld) {
    ...
    switch (Settings::ESP::FilterLocalPlayer::Chams::type) {
        ...
        case ChamsType::GLOWF:
            ...
            break;
        case ChamsType::PULSE:
            Fake_meterial = material->FindMaterial("models/inventory_items/dogtags/dogtags_outline",
                                                   TEXTURE_GROUP_MODEL);
            break;
        case ChamsType::GLOW:
        ...
        case ChamsType::ANIMATED:
            Fake_meterial = material->FindMaterial("models/inventory_items/music_kit/darude_01/mp3_detail",
                                                   TEXTURE_GROUP_MODEL);
            break;
    ...
    */
    ...
    if (Settings::ESP::SyncFake || (localplayer->GetVelocity().Length2D() < 2.05f && Settings::FakeLag::enabled) ||
        (Settings::AntiAim::FakeDuck::enabled && inputSystem->IsButtonDown(Settings::AntiAim::FakeDuck::fakeDuckKey)))
...
static void DrawWeapon(const ModelRenderInfo_t &pInfo) {
    if (!Settings::ESP::Chams::Weapon::enabled)
        return;
    //turn weapon chams off while scoped so you can actually see with COD Guns.
    C_BasePlayer *localplayer = (C_BasePlayer *) entityList->GetClientEntity(engine->GetLocalPlayer());
...
static void
DrawRecord(void *thisptr, void *context, void *state, const ModelRenderInfo_t &pInfo, matrix3x4_t *pCustomBoneToWorld) {
...
static void
DrawArms(void *thisptr, void *context, void *state, const ModelRenderInfo_t &pInfo, matrix3x4_t *pCustomBoneToWorld) {
    ...
    mat->SetMaterialVarFlag(MATERIAL_VAR_WIREFRAME, Settings::ESP::Chams::Arms::type == ChamsType::WIREFRAME ||
                                                    Settings::ESP::Chams::Arms::type == ChamsType::ANIMATED);
...
void Chams::DrawModelExecute(void *thisptr, void *context, void *state, const ModelRenderInfo_t &pInfo,
    ...
    if (!materialsCreated) {
        materialChamsPearl = Util::CreateMaterial2(XORSTR("VertexLitGeneric"),
